 ...
 * @创建时间   2017/2/6 14:17
 * @本类描述	   SpannableStringUtils工具类
 * @内容说明   TextView是用来显示文本的,有时需要给TextView中的个别字设置为超链接,
 *             或者设置个别字的颜色、字体等,
 *             那就需要用到Spannable对象,可以借助Spannable对象实现以上设置
 * @补充内容
 *
 * ---------------------------------
 * @更新时间
    ...
    public static class Builder {
        ...
        private void setSpan() {
            ...
            if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
                if (imageIsBitmap) {
                    mBuilder.setSpan(new CustomImageSpan(GlobalUtils.getApp(), bitmap, align), start, end, flag);
                    bitmap = null;
                    imageIsBitmap = false;
                    ...
                    mBuilder.setSpan(new CustomImageSpan(GlobalUtils.getApp(), uri, align), start, end, flag);
                    uri = null;
                    imageIsUri = false;
                } else {
                    mBuilder.setSpan(new CustomImageSpan(GlobalUtils.getApp(), resourceId, align), start, end, flag);
                    resourceId = 0;
